...
class MeanEmbeddingModel:
    ...
    treatment_kernel_fn = BinaryKernel()
    ...
    def fit_alpha(self, treatment_nl: np.ndarray, covariates_nl: np.ndarray, cfact_val: float):
        self.treatment_nl = treatment_nl
        self.covariates_nl = covariates_nl
        self.n = self.treatment_nl.shape[0]
        self.cfact_val = np.ones((self.n, 1)) * cfact_val
        ...
        K2 = self.treatment_kernel_fn.cal_kernel_mat(self.cfact_val, self.treatment_nl).T * covariates_kernel_nl
        K3 = K2.T
        K4 = self.treatment_kernel_fn.cal_kernel_mat(self.cfact_val, self.cfact_val) * covariates_kernel_nl
    ...
    def predict(self, treatment_l: np.ndarray, covariates_l: np.ndarray) -> np.ndarray:
        ...
        u_bottom = (self.treatment_kernel_fn.cal_kernel_mat(self.cfact_val, treatment_l)) * covariates_kernel_cross
        u = np.block([[u_top],
                      [u_bottom]])
        return (self.w.T @ u).T